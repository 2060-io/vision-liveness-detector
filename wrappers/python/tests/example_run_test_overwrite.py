...
def main():
    ...
    server_client.set_take_picture_callback(take_picture_callback)  # Frame will be provided by the async server
        ...
        try:
            while True:
                ...
                if key == ord('q'):
                    ...
                    overwrite_text = (
                        "This is the Overwrite text message in multiple lines, "
                        "so I have to write a very big text to have multiple lines"
                    )
                    server_client.set_overwrite_text(overwrite_text)
                elif key == ord('w'):
                    # Set warning message
                    warning_text = (
                        "This is the Warning text message in multiple lines, "
                        "so I have to write a very big text to have multiple lines"
                    )
